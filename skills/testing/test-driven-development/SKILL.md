...
```bash
./gradlew test --tests "com.example.RetryServiceTest"
```
...
<Good>
```java
public <T> T retryOperation(Callable<T> fn) throws Exception {
    for (int i = 0; i < 3; i++) {
        try {
            return fn.call();
        } catch (Exception e) {
...
**Verify RED**
```bash
$ ./gradlew test --tests "com.example.UserServiceTest#rejectsEmptyEmail"
FAILED: expected IllegalArgumentException but no exception was thrown
```
